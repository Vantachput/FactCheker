...
from database.db_manager import check_and_increment_limit, _db_conn, LIMITS

@pytest.mark.asyncio
async def test_check_and_increment_limit_admin_bypass():
    """Тест: Адмін ігнорує ліміт."""
    with patch('database.db_manager._db_conn', None):  # Не залежить від БД
...
async def test_check_and_increment_limit_new_user_new_day():
    ...
    global _db_conn
    _db_conn = await aiosqlite.connect(':memory:')
    await _db_conn.execute('CREATE TABLE usage (user_id INTEGER, model_name TEXT, count INTEGER, last_reset TEXT)')
    ...
    # Перевірка БД
    cursor = await _db_conn.execute('SELECT count FROM usage WHERE user_id=123 AND model_name="sonar-reasoning-pro"')
...
async def test_check_and_increment_limit_exceed_limit():
    ...
    await _db_conn.execute('INSERT INTO usage VALUES (123, "sonar-reasoning-pro", 1, ?)', (today,))
    await _db_conn.commit()

    with patch('db_manager.LIMITS', {"sonar-reasoning-pro": 1}):
        allowed, limit = await check_and_increment_limit(123, "sonar-reasoning-pro", 0)
...
async def test_check_and_increment_limit_exception_handling():
    ...
    with patch('aiosqlite.Connection.execute', side_effect=Exception("SQL error")), \
